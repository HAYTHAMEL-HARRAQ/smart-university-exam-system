...
import { initializeOraclePool, testOracleConnection, createOracleTables, migrateDataToOracle } from "../oracleDb";
...
async function startServer() {
  ...
  // Manual CORS handling for credentials
  ...
  // Register simple auth routes (no OAuth)
  ...
  initializeOraclePool()
    .then(async () => {
      ...
      if (isConnected) {
        ...
        // Optionally migrate data from MySQL to Oracle
        // Uncomment the next line if you want to migrate existing data
        // await migrateDataToOracle();
        
        console.log('✅ Oracle setup complete');
      } else {
        console.log('⚠️ Oracle database not available, continuing with MySQL only');
      }
    })
    .catch(err => {
      console.error('❌ Oracle initialization error:', err);
      console.log('⚠️ Continuing with MySQL only');
    });
